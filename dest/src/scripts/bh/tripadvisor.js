...
class TripAdvisor {
    constructor(rc, start, destination) {
        ...
        this.route = utils_1.lazily(() => this.rc.findRoute(this.start.coords, this.destination.coords));
    }
    get score() {
    ...
    journeys() {
        const systems = this.route().hops.flatMap(hop => [hop.blackhole, hop.exit])
            .unshift(this.start)
