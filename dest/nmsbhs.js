...
commander
    ...
    .option("-d, --destination [value]", "destination coordinates", coordinates)
    .action((options) => {
    const best = List(options.start)
        .map((start, index) => {
        return new TripAdvisor(new RouteCalculator(allHops), { label: `START[${index + 1}]`, coords: start }, { label: "DESTINATION", coords: options.destination });
    })
        .minBy(ta => {
